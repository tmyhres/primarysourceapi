...
import com.goodnews.primarysource.config.KafkaProperties;
...
class PrimarysourceapiKafkaResourceIT {
    ...
    void setup() {
        ...
        PrimarysourceapiKafkaResource kafkaResource = new PrimarysourceapiKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/primarysourceapi-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/primarysourceapi-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
